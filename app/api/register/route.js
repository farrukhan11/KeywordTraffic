...
const schema = z.object({ name: z.string().min(2).max(100), email: z.string().email(), password: z.string().min(8) });
...
export async function POST(request) {
  try {
    ...
    await User.create({ name: body.name, email, passwordHash: await bcrypt.hash(body.password, 12) });
